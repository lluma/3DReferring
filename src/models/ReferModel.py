...
class ReferModel(nn.Module):
    ...
    def forward(self, pc, expr):
        ...
        # local: BS x point feature size(128) x points(8192)
        ...
        # BS x points(8192) x point feature size(128)
        inv_local_features = local_features.transpose(1, 2)
        
        # BS x points(8192) x point feature size(7)
        expanded_pred_box = pred_box.unsqueeze(1).expand(pred_box.size(0), inv_local_features.size(1), pred_box.size(1))
        
        # BS x points(8192) x point feature size(135)
        combined_features = torch.cat((inv_local_features, expanded_pred_box), dim=2)
         
        combined_features = combined_features.transpose(1, 2) # BS x point feature size(135) x points(8192)
        instance_points = self.inst_seg_pred_layers(combined_features) # BS x point feature size(1) x points(8192)
        
        ### Output
        pred_confidence = torch.sigmoid(pred_box[:,-1:])
        pred_box = pred_box[:,:-1]
        instance_points = instance_points.view((-1, instance_points.size(2))) # BS x points(8192)
...
if __name__ == '__main__':
    ...
    out1, out2, out3 = model(torch.rand(4,8192,6).cuda(), torch.randint(low=0, high=embedder.get_vocabulary_size(), size=(4,20)).cuda())
    print ('Output:')
